...
public class BmwBackendEmail {
    public static void main(String[] args) throws Exception {
        ...
        String portEnv = System.getenv("PORT");
        int port = (portEnv != null) ? Integer.parseInt(portEnv) : 8081; // Using 8081 if running locally alongside the chatbot
        
        HttpServer server = HttpServer.create(new InetSocketAddress(port), 0);
        
        // Maps the registration endpoint route
        server.createContext("/api/register", new RegisterEmailHandler());
        
        server.setExecutor(null);
        System.out.println("BMW Email Service actively listening on port: " + port);
        server.start();
    }
        ...
        public void handle(HttpExchange exchange) throws IOException {
            ...
            if ("POST".equalsIgnoreCase(exchange.getRequestMethod())) {
                try {
                    ...
                    // Grab Mailjet credentials from Render environment variables
                    ...
                    // NOTE: Change "your_verified_email@gmail.com" to the exact email you verified on Mailjet!
